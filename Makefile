...
# --- General Targets ---
.PHONY: all help license check docker start stop restart clean deep-clean doc check-api check-dashboard local reset start-server start-agent local-reset start-docker-compose stop-docker-compose images docker-compose
...
fabric: ##@Build Build fabric image.
	docker build -t $(DOCKER_NS)/fabric:$(HLF_FABRIC_VERSION) ./src/nodes/hyperledger-fabric
...
check-api: ##@Test Run API tests with newman
	@cd tests/postman && docker compose up --abort-on-container-exit
